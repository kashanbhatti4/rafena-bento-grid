...
var woo_notification = {
    ...
    ajax_get_data: function() {
        ...
        this.ajax_url && (e = this.id ? "&id=" + this.id : "", jQuery.ajax({
            type: "POST",
            data: "action=woonotification_get_product" + e + "&nonce=" + woo_notification.nonce,
            url: this.ajax_url,
            success: function(e) {
                var t = JSON.parse(e);
    ...
    message_show: function() {
        ...
        if (!(this.total <= e)) {
            ...
            t.hasClass(this.hidden_effect) && t.removeClass(this.hidden_effect), t.addClass(this.display_effect).css("display", "inline-grid"), this.wn_popup = setTimeout(function() {
    ...
    get_product: function() {
        ...
        if ("undefined" != s && s && c && s.length > 0 && c.length > 0) {
            ...
            var p = "<small>" + _woocommerce_notification_params.str_about + " " + n + " " + _woocommerce_notification_params.str_ago + " </small>",
                l = this.message_custom,
                h = "";
            u.thumb ? (jQuery("#message-purchased").addClass("wn-product-with-image").removeClass("wn-product-without-image"), "1" === r ? (h = '<a class="wn-notification-image-wrapper" ', "1" === _ && (h += 'target="_blank"'), h += ' href="' + u.url + '"><img class="wn-notification-image" src="' + u.thumb + '" alt="' + u.thumb_alt + '"></a>') : h = '<span class="wn-notification-image-wrapper"><img class="wn-notification-image" src="' + u.thumb + '" alt="' + u.thumb_alt + '"></span>') : jQuery("#message-purchased").addClass("wn-product-without-image").removeClass("wn-product-with-image"), l = l.replace("{number}", this.random(this.message_number_min, this.message_number_max));
            for (var w = this.shortcodes, g = [e, i, t, o, d, f, p, l], v = m, y = w.length - 1; y >= 0; y--) v = v.replace(w[y], g[y]);
            var b = "";
            parseInt(this.show_close) > 0 && (b = '<div id="notify-close"></div>');
            var C = h + '<p class="wn-notification-message-container">' + v + "</p>";
