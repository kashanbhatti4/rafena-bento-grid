(function($) {
    ...
    var JetSearch = {
        ...
        widgetAjaxSearch: function($scope) {
            ...
            $scope.find(settings.inputClass).on('focus.jetSearchSlick input.jetSearchSlick', function() {
        ...
        initElementsHandlers: function($selector) {
            ...
            $selector.find('[data-element_type]').each(function() {
                var excludeWidgets = ['jet-form-builder-form.default', 'nav-menu.default', 'jet-woo-product-gallery-slider.default', 'accordion.default'];
                var $this = $(this),
        ...
        pauseAllSlick: function($root) {
            this.toggleSlickAutoplay($root || $(document), !1)
        },
        playAllSlick: function($root) {
            setTimeout(() => this.toggleSlickAutoplay($root || $(document), !0), 60)
        },
        ...
        addSuggestionFromResultAreaItem: function(itemWrapper, itemLink, itemTitle, itemUrlAttr) {
            itemLink.on('click', function(e) {
                ...
                JetSearch.setFormSuggestion(value, !1, url)
            })
        }
    };
    ...
    $.fn.jetAjaxSearch = function(args) {
        ...
        if (!self.isInit) {
            ...
            self.successCallback = function(response) {
                ...
                searchSourcesBeforePosts = searchSources.filter(item => item.priority < 0 && '' != item.content).sort((a, b) => a.priority - b.priority);
                searchSourcesAfterPosts = searchSources.filter(item => item.priority > 0 && '' != item.content).sort((a, b) => a.priority - b.priority);
            ...
            self.maybeHasListing = function(item) {
                if (typeof item.listing_template !== 'undefined' && !0 === item.listing_template) {
                    Promise.all(JetSearch.assetsPromises).then(function() {
                        JetSearch.initElementsHandlers(resultsListInner);
                        JetSearch.reinitBricksScripts(self);
                        JetSearch.assetsPromises = []
            ...
            self.ajaxSendData = function(sendData, lang = '') {
                ...
                Object.keys(sendData).forEach(key => {
                    if (sendData[key] !== '' && sendData[key] !== null && sendData[key] !== undefined && !(Array.isArray(sendData[key]) && sendData[key].length === 0)) {
